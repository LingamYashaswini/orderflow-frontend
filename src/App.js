...
function App() {
  ...
  const buildOrdersPDF = (title, list) => {
    ...
    const rows = sorted.map(o => `<tr><td style="padding:8px;white-space:nowrap">${formatDate(o.date)}</td><td style="padding:8px">${o.distributorId?.name || '-'}</td><td style="padding:8px">${o.invoiceNumber}</td><td style="padding:8px">Rs.${Number(o.amount).toLocaleString('en-IN')}</td></tr>`).join('');
  ...
  const buildPaymentsPDF = (title, list) => {
    ...
    const rows = sorted.map(p => `<tr><td style="padding:8px;white-space:nowrap">${formatDate(p.date)}</td><td style="padding:8px">${paymentName(p)}</td><td style="padding:8px">Rs.${Number(p.amount).toLocaleString('en-IN')}</td></tr>`).join('');
  ...
  const generateInvoiceSummaryPDF = () => {
    ...
    sortedDists.forEach(d => {
      ...
      distOrders.forEach(o => { rows += `<tr><td style="padding:6px 8px"></td><td style="padding:6px 8px;white-space:nowrap">${formatDate(o.date)}</td><td style="padding:6px 8px">${o.invoiceNumber}</td><td style="padding:6px 8px">Rs.${Number(o.amount).toLocaleString('en-IN')}</td></tr>`; });
  ...
  const generatePaymentSummaryPDF = () => {
    ...
    sortedDists.forEach(d => {
      ...
      dp.forEach(p => { rows += `<tr><td style="padding:6px 8px"></td><td style="padding:6px 8px;white-space:nowrap">${formatDate(p.date)}</td><td style="padding:6px 8px">Rs.${Number(p.amount).toLocaleString('en-IN')}</td></tr>`; });
  ...
  const filteredDists = distributors.filter(d => d.name.toLowerCase().startsWith(search.toLowerCase()) || search === '').sort((a,b) => a.name.localeCompare(b.name));
              ...
              <div style={{ flex: 1, overflowY: 'auto' }}>
                {[...distributors].sort((a,b) => a.name.localeCompare(b.name)).map((d, idx) => (
                  <div key={d._id} onClick={() => { setSelectedDist(d); setView('distributor'); fetchOrders(d._id); setDistOrderSelectedIds([]); }}
                    ...
                    <div style={{ fontWeight: 700, color: '#3095e2', minWidth: 24 }}>{idx + 1}.</div>
                    <div style={{ flex: 1 }}>
          ...
          {view === 'allOrders' && (
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
                ...
                {selectedOrderIds.length > 0 && <button onClick={() => { const list = allOrders.filter(o => selectedOrderIds.includes(o._id)); if (!list.length) { alert('Select at least one'); return; } buildOrdersPDF('Selected Purchases', list); }} style={btnOutline}>📄 Download Selected ({selectedOrderIds.length})</button>}
              </div>
                ...
                <tbody>
                  {[...allOrders].sort((a,b) => new Date(a.date) - new Date(b.date)).map(o => (
                    <tr key={o._id} style={{ borderBottom: '1px solid #eee' }}>
                      <td style={{ padding: 10 }}><input type="checkbox" checked={selectedOrderIds.includes(o._id)} onChange={() => toggleSelect(selectedOrderIds, setSelectedOrderIds, o._id)} style={{ cursor: 'pointer' }}/></td>
                ...
                <tbody>
                  {(() => {
                    ...
                    sortedDists.forEach(d => {
                      ...
                      blocks.push(<tr key={`h-${d._id}`}><td colSpan={3} style={{ padding: '10px 8px 4px', fontWeight: 700, fontSize: 14, color: '#3FA0E8' }}>{d.name}</td></tr>);
                      distOrders.forEach(o => blocks.push(<tr key={o._id} style={{ borderBottom: '1px solid #f5f5f5' }}><td style={{ padding: 8 }}>{formatDate(o.date)}</td><td style={{ padding: 8 }}>{o.invoiceNumber}</td><td style={{ padding: 8 }}>Rs.{Number(o.amount).toLocaleString('en-IN')}</td></tr>));
                      blocks.push(<tr key={`sub-${d._id}`} style={{ borderBottom: '2px solid #eee' }}><td></td><td style={{ padding: 8, fontWeight: 600 }}>Subtotal</td><td style={{ padding: 8, fontWeight: 600 }}>Rs.{distTotal.toLocaleString('en-IN')}</td></tr>);
                    });
                    return blocks;
                  })()}
                </tbody>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
              <div style={{ flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
                  ...
                  {selectedPaymentIds.length > 0 && <button onClick={() => { const list = payments.filter(p => selectedPaymentIds.includes(p._id)); if (!list.length) { alert('Select at least one'); return; } buildPaymentsPDF('Selected Payments', list); }} style={btnOutline}>📄 Download Selected ({selectedPaymentIds.length})</button>}
                </div>
                  ...
                  <tbody>
                    {[...payments].sort((a,b) => new Date(a.date) - new Date(b.date)).map(p => (
                      <tr key={p._id} style={{ borderBottom: '1px solid #eee' }}>
                        <td style={{ padding: 10 }}><input type="checkbox" checked={selectedPaymentIds.includes(p._id)} onChange={() => toggleSelect(selectedPaymentIds, setSelectedPaymentIds, p._id)} style={{ cursor: 'pointer' }}/></td>
                        ...
                        <td style={{ padding: 10 }}>
                          <div style={{ display: 'flex', gap: 6 }}>
                            <button onClick={() => openEditPayment(p)} style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid #ccc', cursor: 'pointer', background: '#fff', fontSize: 12 }}>Edit</button>
                            <button onClick={() => handleDeletePayment(p._id)} style={{ padding: '4px 8px', borderRadius: 6, border: '1px solid #ccc', cursor: 'pointer', background: '#fff', color: 'red', display: 'flex', alignItems: 'center' }}><TrashIcon/></button>
                          </div>
                        </td>
                      </tr>
                    ))}
                ...
                <tbody>
                  {(() => {
                    ...
                    sortedDists.forEach(d => {
                      ...
                      blocks.push(<tr key={`h-${d._id}`}><td colSpan={2} style={{ padding: '10px 8px 4px', fontWeight: 700, fontSize: 14, color: '#3FA0E8' }}>{d.name}</td></tr>);
                      dp.forEach(p => blocks.push(<tr key={p._id} style={{ borderBottom: '1px solid #f5f5f5' }}><td style={{ padding: 8 }}>{formatDate(p.date)}</td><td style={{ padding: 8 }}>Rs.{Number(p.amount).toLocaleString('en-IN')}</td></tr>));
                      blocks.push(<tr key={`sub-${d._id}`} style={{ borderBottom: '2px solid #eee' }}><td style={{ padding: 8, fontWeight: 600 }}>Subtotal</td><td style={{ padding: 8, fontWeight: 600 }}>Rs.{dt.toLocaleString('en-IN')}</td></tr>);
                    });
                    return blocks;
                  })()}
                </tbody>
            ...
            <div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  {distOrderSelectedIds.length > 0 && <button onClick={() => { const list = orders.filter(o => distOrderSelectedIds.includes(o._id)).map(o => ({ ...o, distributorId: selectedDist })); if (!list.length) { alert('Select at least one'); return; } buildOrdersPDF('Selected - ' + selectedDist.name, list); }} style={btnOutline}>📄 Download Selected ({distOrderSelectedIds.length})</button>}
                </div>
              </div>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                ...
                <tbody>
                  {orders.map(o => (
                    <tr key={o._id} style={{ borderBottom: '1px solid #eee' }}>
                      <td style={{ padding: 10 }}><input type="checkbox" checked={distOrderSelectedIds.includes(o._id)} onChange={() => toggleSelect(distOrderSelectedIds, setDistOrderSelectedIds, o._id)} style={{ cursor: 'pointer' }}/></td>
                      <td style={{ padding: 10 }}>{formatDate(o.date)}</td>
                      <td style={{ padding: 10 }}>{o.invoiceNumber}</td>
                      <td style={{ padding: 10, fontWeight: 600 }}>Rs.{Number(o.amount).toLocaleString('en-IN')}</td>
                      <td style={{ padding: 10 }}>
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button onClick={() => openEditOrder(o)} style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid #ccc', cursor: 'pointer', background: '#fff', fontSize: 12 }}>Edit</button>
                          <button onClick={() => buildOrdersPDF(selectedDist.name, [{ ...o, distributorId: selectedDist }])} style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid #3FA0E8', cursor: 'pointer', background: '#3FA0E8', color: '#fff', fontSize: 12 }}>PDF</button>
                          <button onClick={() => handleDeleteOrder(o._id)} style={{ padding: '4px 8px', borderRadius: 6, border: '1px solid #ccc', cursor: 'pointer', background: '#fff', color: 'red', display: 'flex', alignItems: 'center' }}><TrashIcon/></button>
                        </div>
                      </td>
                    </tr>
                  ))}
                ...
                <tfoot>
                  <tr style={{ borderTop: '2px solid #eee', background: '#f9f9f9' }}>
                    ...
                    <td style={{ padding: 12, fontWeight: 700, fontSize: 15, color: '#3FA0E8' }}>Rs.{orders.reduce((s,o) => s + Number(o.amount), 0).toLocaleString('en-IN')}</td>
                    <td></td>
                  </tr>
                </tfoot>
              </table>
