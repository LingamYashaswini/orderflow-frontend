    ...
    <div style={{ fontFamily: 'sans-serif', display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 28, background: '#F0F8FE' }}>
                ...
                <tbody>
                  {(() => {
                    ...
                    sortedDists.forEach(d => {
                      ...
                      blocks.push(<tr key={`h-${d._id}`}><td colSpan={3} style={{ padding: '4px 4px 2px', fontWeight: 700, fontSize: 14, color: '#3FA0E8' }}>{d.name}</td></tr>);
                      distOrders.forEach(o => blocks.push(<tr key={o._id} style={{ borderBottom: '1px solid #f5f5f5' }}><td style={{ padding: '1px 4px' }}>{formatDate(o.date)}</td><td style={{ padding: '1px 4px' }}>{o.invoiceNumber}</td><td style={{ padding: '1px 4px' }}>Rs.{Number(o.amount).toLocaleString('en-IN')}</td></tr>));
                      blocks.push(<tr key={`sub-${d._id}`} style={{ borderBottom: '2px solid #eee' }}><td></td><td style={{ padding: '1px 4px', fontWeight: 600 }}>Subtotal</td><td style={{ padding: '1px 4px', fontWeight: 600 }}>Rs.{distTotal.toLocaleString('en-IN')}</td></tr>);
                    });
                    return blocks;
                  })()}
                </tbody>
          ...
          {/* ===== DISTRIBUTOR PAYMENTS (SUPER COMPACT) ===== */}
          {view === 'payments' && (
            <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
              <div style={{ flexShrink: 0 }}>
                <button onClick={() => setView('dashboard')} style={btnBack}>← Back</button>
